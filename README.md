...
2. Вью реагирует на действие пользователя и генерирует событие card:select.
3. В обработчике клика вызывается событие card:select с данными товара (IItem).
4. Презентер обрабатывает событие и вызывает метод модели для изменения данных:
- Обработчик events.on('card:select', ...) проверяет, есть ли товар уже в заказе через orderData.checkItem(item.id).
- Если товара нет — вызывает orderData.addItem(item.id, item.price).
- Если товар есть — вызывает orderData.removeItem(item.id, item.price).
5. Модель изменяет данные и генерирует событие:
- Методы addItem / removeItem внутри OrderModel обновляют объект заказа и вызывают событие order:changed.
6. Презентер обрабатывает событие и вызывает рендер Вью, передавая данные из модели:
- Обработчик events.on('order:changed', ...) обновляет счетчик корзины на странице (page.counter = cartCount).
- Обновляет валидность корзины (basket.valid = !!cartCount).
- Обновляет массив карточек товаров в корзине (basket.items = orderData.order.items.map(...)).
- Пересчитывает итоговую сумму (basket.total = orderData.getTotal()).
7. Вью перерисовывается, отображая обновленные данные:
- Пользователь видит обновленное количество товаров в корзине, изменившиеся карточки товаров и актуальную сумму заказа.